...
import { FaEye, FaEdit, FaTrash, FaPhone, FaEnvelope, FaUser, FaFolderOpen } from 'react-icons/fa';
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 mt-2">
        <div className="flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            <h3 className="text-xl font-bold text-text-title leading-tight truncate pr-20" title={`${owner.name} ${owner.surname}`}>
      ...
      <div className="space-y-3 mb-6 grow">
        <div className="flex items-center gap-3 text-text-body bg-bg-main/40 p-2.5 rounded-lg border border-border/30">
          <FaPhone className="text-secondary shrink-0" size={14} />
          <span className="text-sm font-medium truncate">
            {owner.phone || 'Nessun telefono'}
          </span>
        </div>
        <div className="flex items-center gap-3 text-text-body bg-bg-main/40 p-2.5 rounded-lg border border-border/30">
          <FaEnvelope className="text-secondary shrink-0" size={14} />
          <span className="text-sm font-medium truncate" title={owner.email}>
            {owner.email}
          </span>
        </div>
      </div>
